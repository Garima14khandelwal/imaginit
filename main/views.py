...
class Meffects(TemplateView):
    ...
    def get(self, requests):
        """Return all the effects available."""
        return HttpResponse(content=[meffect.keys()])
...
def Index(request):
    return render(request,'main/hindex.html',{})

class Manip(LoginRequiredMixin, TemplateView):
    """Create dashboard and save new upload."""

    login_required = True
    template_name = 'main/manip.html'
